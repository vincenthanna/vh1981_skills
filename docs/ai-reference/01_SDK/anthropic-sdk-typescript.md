...
MCP Helpers
This SDK provides helpers for integrating with Model Context Protocol (MCP) servers. These helpers convert MCP types to Anthropic API types, reducing boilerplate when working with MCP tools, prompts, and resources.

Note: The Claude API also supports an mcp_servers parameter that lets Claude connect directly to remote MCP servers.
...
Long Requests
ImportantWe highly encourage you use the streaming Messages API for longer running requests.
...
An expected request latency longer than the timeout for a non-streaming request
will result in the client terminating the connection and retrying without receiving a response.
When supported by the fetch implementation, we set a TCP socket keep-alive option in order
to reduce the impact of idle connection timeouts on some networks.
This can be overriden by configuring a custom proxy.
Auto-pagination
...
console.log(message.content);
Logging
ImportantAll log messages are intended for debugging only. The format and content of log messages
may change between releases.

Log levels
The log level can be configured in two ways:

Via the ANTHROPIC_LOG environment variable
...
Beta Features
We introduce beta features before they are generally available to get early feedback and test new functionality. You can check the availability of all of Claude's capabilities and tools here.
