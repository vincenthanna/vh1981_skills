...
- composer SSOT: `plugins/prompts-pack/lib/prompt-composer-system/CLAUDE.md`. v1.2 default flow: Conv1 Compose → Conv2 Execute → Conv3 Evaluate.
...
- 표준 머리말 규약(`CLAUDE.md` "표준 머리말 형식") 위반 파일 점검:
  - 자동 검사: `for f in plugins/prompts-pack/lib/prompt-composer-system/*.md plugins/prompts-pack/lib/prompt-composer-system/builder/*.md plugins/prompts-pack/lib/prompt-composer-system/components/*.md; do head -3 "$f" | grep -q "^> \*\*무엇\*\*" || echo "MISSING_HEADER: $f"; done`  (★ `builder/` 포함 — `f86cc2a` 리팩터 반영, §5.1 경로와 일치)
...
- `rfc-writing-template ↔ speckit-spec-generation` 상호 배타 4-SSOT 정합성:
  - 자동 검사: `grep -l "rfc-writing-template" plugins/prompts-pack/lib/prompt-composer-system/components/speckit-spec-generation.md plugins/prompts-pack/lib/prompt-composer-system/CLAUDE.md plugins/prompts-pack/lib/prompt-composer-system/builder/prompt-component-router.md` → 3 hit 기대.
- 각 component metadata의 `layer` 필드 echo 여부:
  - 자동 검사: `grep -L "^| layer |" plugins/prompts-pack/lib/prompt-composer-system/components/*.md` → 0 hit 기대 (모두 layer 필드 보유).
...
| # | 항목 | 검사 명령 (PASS 조건) |
|---|---|---|
| S1 | `## 0. Router 등록 metadata` section 미보유 | `grep -c "^## 0\. Router 등록 metadata" plugins/prompts-pack/lib/prompt-composer-system/audit-composer-system.md` == 0 |
| S2 | router §1/§2/§7/composer 시리즈 등록 0건 | `grep -l "audit-composer-system" plugins/prompts-pack/lib/prompt-composer-system/builder/prompt-component-router.md plugins/prompts-pack/lib/prompt-composer-system/builder/optimized-prompt-composer.md` == 0 hit |
| S3 | 머리말 ⚠️ 경고 보유 | `head -10 plugins/prompts-pack/lib/prompt-composer-system/audit-composer-system.md \| grep -c "⚠️"` ≥ 2 |
| S4 | self-eval 경고 명시 | `grep -c "self-eval 경고\|블라인드 독립 채점" plugins/prompts-pack/lib/prompt-composer-system/audit-composer-system.md` ≥ 2 |
| S5 | Step 0-5 + §6 SSOT 표 보유 | `grep -cE "^- \*\*Step [0-5]\|\*\*G[0-5]\*\*" plugins/prompts-pack/lib/prompt-composer-system/audit-composer-system.md` ≥ 10  (★ N4 교정: table-안전하게 *alternation만* 사용 — Step 헤더 6 + Gate 행 6 = 12. 직전 패턴은 ERE에서 `\|`를 literal pipe로 처리해 false 0이었음) |
...
### 4.1 `plugins/prompts-pack/lib/prompt-composer-system/builder/prompt-evaluation-rubric.md` (audit criteria SSOT)
...
### 4.2 `plugins/prompts-pack/lib/prompt-composer-system/components/multi-agent-analysis-template.md` (3 test 같은 criteria 평가)
...
### 4.3 `plugins/prompts-pack/lib/prompt-composer-system/components/agent-role-dictionary.md` (자기 점검 anchoring 방지)
...
### 5.1 Repo 내부 SSOT
- `plugins/prompts-pack/lib/prompt-composer-system/CLAUDE.md`
- `plugins/prompts-pack/lib/prompt-composer-system/builder/optimized-prompt-composer.md`
- `plugins/prompts-pack/lib/prompt-composer-system/builder/prompt-component-router.md`
- `plugins/prompts-pack/lib/prompt-composer-system/builder/task-spec-template.md`
- `plugins/prompts-pack/lib/prompt-composer-system/builder/context-injection-patterns.md`
- `plugins/prompts-pack/lib/prompt-composer-system/builder/prompt-evaluation-rubric.md`
- `plugins/prompts-pack/lib/prompt-composer-system/trigger-prompts.md`
- `plugins/prompts-pack/lib/prompt-composer-system/trigger.md` (인터페이스 변형)
- `plugins/prompts-pack/lib/prompt-composer-system/component-discovery-{collect,approve,ledger}.md`
- `plugins/prompts-pack/lib/prompt-composer-system/components/` 7파일
...
|---|---|---|---|
| **G0** | SSOT root 7 + builder/ 5 + components/ 7 파일 read 0 실패 + 인벤토리 표 산출 | `ls plugins/prompts-pack/lib/prompt-composer-system/*.md plugins/prompts-pack/lib/prompt-composer-system/builder/*.md plugins/prompts-pack/lib/prompt-composer-system/components/*.md \| wc -l` ≥ 18  (★ N3 교정: `builder/` 포함 — `f86cc2a` 리팩터 반영; 미포함 시 14로 false FAIL) | **1파일 read 실패** → 그 파일 path를 사용자에 보고 + audit 결과에 `[ASSUMPTION:<file> 미read]` 격하 (graceful degradation, 진행 계속). **2파일 이상 실패** → audit 자체 abort + 사용자 보고 + 차기 run 권고 |
...
| **G4** | CLAUDE.md "진단 도구" 카테고리 echo + diff ≤ 5줄 + router §1/§2/§7/composer 시리즈 등록 0건 | `grep -c "audit-composer-system" plugins/prompts-pack/lib/prompt-composer-system/CLAUDE.md` ≥ 1 AND `grep -l "audit-composer-system" plugins/prompts-pack/lib/prompt-composer-system/builder/prompt-component-router.md plugins/prompts-pack/lib/prompt-composer-system/builder/optimized-prompt-composer.md` == 0 hit | 등록 1자라도 발견 시 즉시 abort |
| **G5** | 8항목 형식(A/B/C/priority/PR 제안/eval/출처/위험) 최종 보고 출력 + 사람 승인 전 mutation 0건 | `git status --short plugins/prompts-pack/lib/prompt-composer-system/ docs/projects/composer-system-audit/` 결과가 read-only 산출만 (audit 보고서 신규 + 디렉토리 mkdir만) | mutation 발생 시 rollback + 사유 보고 |
