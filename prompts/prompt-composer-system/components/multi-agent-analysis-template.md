...
## 0.5. 사용 적합도 — Fit signals / Warning signs
...
## 0.6. Quick-Start (5분 안에 첫 dispatch)
...
- **Orchestrator (메인 세션)**: plan → dispatch → synthesis. 목적/criteria 정의, topic decomposition, subagent 병렬 디스패치, 결과 수집 후 합성(상호의존성·매트릭스·권고).
- **Per-Topic Analyst (subagent × N, 병렬)** = `topic-analyst`: 자체 context window, 동일 framework 적용, 구조화된 결과만 parent에 반환.
- **Baseline Collector (subagent, required, 1회)** = `baseline-collector`: 공통 기반 정보(코드/환경/외부 docs) 수집.

> 데이터 흐름: Orchestrator가 (1) Baseline Collector를 먼저 호출 → 결과 파일 생성 → (2) topic-analyst에게 그 파일 경로(`baseline_ref`)를 전달. analyst는 collector를 직접 호출하지 않는다.
...
> ❌ 태그 없는 사실 주장 금지. 태그 없는 문장은 "의견"으로 분류되어 권고 근거에서 제외된다. (self-application — 이 문서 본문 자체도 가능한 한 같은 규칙을 따른다.)
>
> 📌 **이 §2.1 표가 evidence tag의 정본(SSOT)이다.** 문서 내 다른 위치(§3.1/§3.2/§7 lint/부록 A/§12)에서 evidence tag를 언급할 때는 형식을 재정의하지 말고 "evidence tag 규칙은 §2.1 적용"으로 참조한다.
...
#### 2.3.1 Confidence × Reversibility 매트릭스
...
> ⚠️ top-2 가중합 차이가 plan에서 선언한 임계 이내면(예: 1-5 척도에서 `|차이| < 0.5`) G3 게이트에서 **정성 분석 단락 의무**. 점수 noise를 의사결정으로 채택하지 않도록 강제.
...
> ⚠️ **Frontmatter 검증 의무** — 아래 frontmatter 필드(`memory`, `isolation`, `model`, `tools`) 및 env var(`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS`)는 Claude Code 버전에 따라 동작/지원 여부가 달라진다. **사용 전 공식 docs(§14 참조)로 *현재 버전 지원 여부*를 검증할 것.** 미지원 키는 무시되거나 오류를 일으킬 수 있다. 이 본문의 frontmatter 예시는 검증 전 `[ASSUMPTION]` 상태로 간주하고, 검증 후 `[VERIFIED:webfetch official <URL> accessed:<date>]`로 격상.

> 📐 **Subagent 호출 표기 안내** — 이 template의 `@"<name> (agent)" ...` 표기는 메인 세션 prompt에서 subagent를 호출하는 *서술형 예시*다. 실제 Claude Code에서는 자연어로 "<name> agent에게 …"라고 지시하면 자동으로 dispatch된다.

> 🔗 **읽는 순서** — 각 subagent의 입력 contract는 §4 Step 3의 dispatch prompt 예시와 짝지어 읽으면 흐름이 이해된다.
...
> ⚠️ frontmatter 운영 주의:
> - `memory: project`: 단, 병렬 dispatch 시에는 stateless 모드 권장 — §6.5 참조.
> - `isolation: worktree`: read-only 분석이고 N>10이면 in-process 또는 tempdir 대안 고려 — §6.6.
> - `memory: project`는 docs상 Read/Write/Edit 권한이 자동 활성화될 수 있음. 보안 표면 확인.
> - `Bash`는 mutation 금지(read & analyze only). worktree 격리 실패 대비 안전장치.
...
(메인 세션에서 어떻게 전달하는지는 §4 Step 3 참조)

- `topic_id`: topic의 고유 식별자 (예: "WS1", "model-qwen-30b", "issue-2203")
- `topic_summary`: topic의 1~2문장 요약  ※ "우선 가설"은 전달하지 않음 (anchoring bias 차단)
...
- `baseline_commit`: baseline 수집 시점의 repo HEAD commit hash (drift 감지용)
- `prior_analysis_path` (선택): 재분석 시 이전 결과
- `out_path`: 결과를 저장할 파일 경로 (예: `.analysis/topics/<topic_id>.md`)

`baseline_ref` 파일이 존재하지 않거나 읽을 수 없으면 analyst는 즉시 abort하고 이유를 메인에 반환한다(1줄 룰 예외). baseline 없이 분석을 계속하지 않는다.

## 분석 절차

1. **Baseline 확인**: `baseline_ref`를 먼저 읽어 공통 기반 정보 파악. `git rev-parse HEAD`로 현재 commit과 `baseline_commit` 비교 — 다르면 출력에 `[ASSUMPTION:repo drifted from t0]` tag 자동 부여.
2. **Scope 결정**: 이 topic과 관련된 파일/모듈/외부 docs를 식별. 너무 넓으면 명시적으로 좁힌다.
3. **사실 수집**: Read/Grep/Glob/Bash/WebFetch로 증거 수집. **모든 사실 주장에 evidence tag 부착 (규칙은 §2.1 적용).**
4. **Criteria별 평가**: 각 criterion에 대해 근거와 함께 평가 작성.
5. **결정 후보 도출**: 가능한 결정 옵션 1~3개. 각각에 reversibility tag와 confidence 부여.
5.5. **Counter-evidence 생성**: 도출한 권고에 대해 "이 권고를 뒤집을 가장 강력한 사실/가설은 무엇인가?"를 스스로 생성하고, 무력화 근거를 찾아라. 실질 반례가 없으면 *왜 못 찾았는지*(예: "탐색 범위 밖", "데이터 부족")를 명시. 결과는 출력 #5 (Open Questions, Assumptions, and Counter-Arguments)에 `[COUNTER]` 태그로 기록.
6. **상호의존성 표시**: 다른 topic과 얽힌 부분이 보이면 명시. 양방향 기록 의무 — "M1이 M2에 의존" + "M2가 M1에 의존받음" 양쪽 모두 기재.
...
**Analyst model id**: <model id at run time>  # 재현성
...
- **태그 없는 사실 주장 금지.** evidence tag 규칙은 §2.1 적용.
...
- **민감정보 redaction.** 비밀값/토큰/credential/원문 env value/민감 endpoint는 저장 금지. 환경 변수는 *이름만*, config는 *shape만*, endpoint는 *존재 여부만* 기록.
- **memory hygiene.** run-specific 결론(예: "M1은 DEAD") 저장 금지. durable pattern만 저장 (예: "Bash entry point 없는 Python module은 SCAFFOLD 후보일 수 있다"). 도메인 라벨 의무. 자세한 정책은 §6.5 참조.
~~~
...
- `env_targets`: 경험적 검증 대상 (서버/장비 list, 없으면 empty) — 분석 단위 "topic"과 구분되는 환경 target.
...
## D. Observed constraints / invariants (evidence-backed만)
- 분석 중 주의해야 할 *검증된* 불변식 / 제약 조건. 추정/gotcha는 baseline이 아니라 topic-analyst의 Open Questions로 이관.
~~~

## 규칙

- **모든 사실에 evidence tag 부착.** 규칙은 §2.1 적용 (webfetch는 tier 필수).
- **해석 금지.** baseline은 사실만 모은다. 평가/권고는 topic-analyst와 orchestrator의 몫.
- **민감정보 저장 금지.** 환경 변수는 *이름만*, config는 *shape만*, endpoint는 *존재 여부만*. `.gitignore` 단독 의존 금지 — pre-commit hook 또는 redaction 검사 step 명시.
- **출력 외 verbose 최소화.** 메인으로는 "baseline 저장 완료: `<out_path>`" 1줄. **종료 응답 = read-ready 보장 약속**.
~~~

### 3.3 `.claude/agents/synthesis-reviewer.md` (선택, §부록 A에서 상세)

요약: synthesis 결과를 비판적으로 검토하는 subagent. **언제 켜는가**:
1. topic ≥ 5
...
### 3.4 / 3.5 외부 LLM bridge subagent (선택, multi-LLM 사용 시)

> `codex-bridge` / `gemini-bridge`: 외부 LLM CLI에 task를 위임하고 결과를 파일로 받는 wrapper. evidence-heavy 분석, 2nd opinion, paired analyst 등에 사용 (§3.6, §6.8).

**공통 절차** (두 bridge 동일):
1. `role_prompt_path` + `context_files`를 임시 prompt 파일로 조립 (각 context는 `--- # CONTEXT: <f>` 헤더로 구분).
2. CLI를 read-only로 호출 → 결과를 `out_path`에 저장.
3. exit code 확인 → 실패 시 1회 재시도 → 두 번째도 실패면 `<out_path>.error.txt`에 stderr 기록 후 `STATUS=error` 보고. **CLI 미설치(`command not found`)면 재시도 없이 즉시 §3.6 fallback.**
4. 출력 파일 존재 + 비어있지 않음 검증.
5. 메인에 1줄 보고: `[<bridge>] <role> done → <out_path>` 또는 `FAILED → fallback: claude`.

**공통 규칙**: 답변 해석/요약/편집 금지(그대로 저장). read-only 강제(write/tool 권한 금지). CLI 버전별 옵션 차이는 §10 setup에서 검증.

~~~markdown
---
name: codex-bridge
description: Codex CLI에 prompt를 전송하고 결과 파일을 받는다(해석 없이 그대로 저장). 입력: (role_prompt_path, context_files, out_path, reasoning_effort).
...
# 가능하면 --ephemeral로 세션 저장 방지 (버전별 옵션 차이).
~~~

~~~markdown
---
name: gemini-bridge
description: Gemini CLI에 prompt를 전송하고 JSON output을 `.response`로 파싱해 저장. 입력: (role_prompt_path, context_files, out_path, model).
tools: Bash, Read, Write
model: sonnet
---
# 핵심 호출 (공통 절차 §3.4 참조):
cat /tmp/gemini_prompt.md | gemini --model "<model>" --output-format json \
  -p "stdin의 작업 지시를 그대로 수행하라." | jq -r '.response' > <out_path>
# jq 없으면 §10 setup에서 차단. 모델은 gemini-2.5-pro 권장(긴 context). --yolo 미사용.
~~~

### 3.6 LLM 배정 매핑 (domain-specialty + Graceful degradation)

> **default는 Claude single-LLM**이다 (small scale, topic 3-5개). Multi-LLM은 **escalation 도구** — 활성화 신호(escalation triggers)는 §6.8.1, 운용 패턴 상세는 §6.8 참조.
...
먼저 분석 메타데이터를 명시적으로 작성한다. `.analysis/<run-id>/00_plan.md` 같은 파일로 저장 (실행 단위 불변 디렉토리):

```markdown
# Analysis Plan

## Guardrail check (multi-agent 필요성)
...
## 분류 체계 (Taxonomy)
- <카테고리 1>: <정의 — 예: "DORMANT = 최근 90일 호출 0회">
- <카테고리 2>: ...

## 점수 규약 (Scoring convention)
- 척도: 1-5 (정수)
- weight 합: 1.0
...
## topic list (Topics)
| ID | 요약 |  # "우선 가설" 컬럼은 plan 작성자 메모 — subagent에 전달 금지
|---|---|
| T1 | ... |
| T2 | ... |
...
- manifest: `.analysis/<run-id>/00_manifest.json`
```
...
> Orchestrator는 Step 2의 baseline 파일이 디스크에 flush되어 read 가능 상태인지 확인 후 Step 3 dispatch한다. **baseline-collector subagent의 종료 응답 = read-ready 보장 약속**.
...
> Orchestrator가 병렬 topic 분석을 명시적으로 요청한다 (자동 dispatch 보장 표현 아님). background 옵션 사용 시 permission 제약뿐 아니라 "완료 여부와 silent/incomplete failure를 어떻게 확인할지" 운영 주의사항을 함께 검토할 것.
...
3. Cross-topic dependency 그래프 (mermaid 또는 표)
   - dependency reciprocity check: A→B가 한쪽에만 있으면 양쪽 topic 재-확인
4. 우선순위 ranking + 근거
5. 미해결 가정 / 추가 검증 필요 항목 통합 list
   - 각 topic의 Open Questions를 합치고 cross-topic 새로운 questions를 추가
...
plan_path: .analysis/<run-id>/00_plan.md  # 외적(목적) 일관성 점검
topics_dir: .analysis/<run-id>/topics
```

리뷰 결과 반영 후 최종본 확정. **blocking issue 0이 될 때까지 iterate, 최대 2회 권장 (G4와 연동).**
...
│   ├── 00_manifest.json            # 실행 기록: agent 버전, timestamp, commit hash
...
>
> **default: `.analysis/` 전체를 commit.** 민감정보는 §5.1의 redacted summary만 보관.
...
### 6.1 topic이 동질적이지 않을 때 — 도메인 특화 agent
...
> ⚠️ **Experimental** — `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS`는 실험 기능이며 동작/이름이 변경될 수 있다. production / CI 사용 전 공식 release notes로 현재 상태 확인 필수. `[ASSUMPTION]` 상태로 간주하고 검증 후 사용.

> 단, 대부분의 경우 메인이 모든 결과를 받은 뒤 2차 분석을 dispatch하는 sequential 방식으로 충분하다.

### 6.4 외부 docs가 핵심일 때 — external-research subagent
...
- **모델 id 기록**: 산출물에 `Analyst model id: <model id at run time>` 메타 필드 의무. alias(`sonnet`) 외에 명시 버전 핀 권장.
- **동시성 (보수적 정책)**: 병렬로 실행되는 `topic-analyst`는 **stateless로 운영하고 공유 memory에 쓰기 금지**가 기본 정책. 패턴 학습이 필요하면 (a) 분석 시작 시 read-only 스냅샷 또는 (b) 모든 analyst 완료 후 commit 단계에서만 메인이 일괄 저장.
- **명령 구체화 예시**: `@"topic-analyst (agent)" 이번 "<도메인>" 분석에서 발견한 <패턴 이름>을 장기 기억에 durable pattern으로 저장해줘.`
- **공식 docs 링크**: §11 참고.

### 6.6 Worktree 격리 — 조건부

- **사용 검증**: §10 사용 전 체크리스트에서 "worktree 동작 검증(분기 후 worktree 목록 확인)" 의무.
- **사용 권장 조건**:
...
#### 6.8.1a Agent / round hard cap

| 차원 | cap | 근거 |
|---|---|---|
| per-topic 독립판단 agent | default 1 / high-stakes 3 / **hard max 5** | ≤5 diminishing returns, 3에서 이득 대부분 |
| debate round | 1 critique 기본 / **2-3 pass cap** | 2-3 round 최적, 3 이후 미미 |
| multi-LLM 적용 topic | critical **1-2개 한정** | 3N 호출 폭증 방지 |
| specialist reviewer | 템플릿 모드 **최대 2** | selection bottleneck |

> cap 초과(예: per-topic agent 5 초과)가 필요하다고 판단되면 **본 template에서 중단**하고 `multi-llm-debate-orchestration.md`의 강도 높은 패턴으로 escalate.
...
- ❌ multi-LLM round 무한 진행 — **2-3 round 최적 (0r 75% → 1r 83% → 2r 87.5% → 3r 89%, 3 round 이후 미미)**. 그 이상은 cost만 증가.
...
- ⚠️ **weak-agent caveat**: fallback이 약한 모델이면 diverse mixture가 오히려 성능 저하(aggregation 의존). fallback은 강 agent(claude) 고정 권장.
- ⚠️ **selection 병목**: agent 불일치 시 단순 majority 금지 — evidence 강도로 결정.
...
| **G3: Synthesis** | 모든 권고에 reversibility + confidence + 근거 요약. 점수 차이 임계 미만이면 정성 분석 단락 존재. 산출물 commit 완료. 권고 → 실행 mapping의 trace ID 의무 (예: `synthesis-2026-05-19#R3`). | synthesis 재합성 |
...
### Quality lint — 무태그 문장 카운트

synthesis-reviewer는 다음을 blocking metric으로 사용 (evidence tag 형식은 §2.1):
- **무태그 사실 문장 수**: 마침표/불릿로 끝나는 사실형 서술 중 evidence tag가 없으면 fail. 1건이라도 있으면 G4 fail.
- **webfetch tag re-fetch sampling**: webfetch tag의 access date + URL을 무작위 N개(예: 5개) 재-fetch하여 동일 사실 확인. fetch 실패 또는 내용 불일치 시 즉시 `[ASSUMPTION]`으로 강등.
...
- **Low confidence + IRREVERSIBLE 권고를 그대로 채택**: 단일 Low가 항상 금지는 아니지만, IRREVERSIBLE과 결합되면 매트릭스(§2.3.1)에 따라 PoC 필수.
- **상호의존성 누락**: topic 단위 권고만 합치고 cross-effect 무시 → reciprocity check.
...
| 시나리오 | 목적 / topic | criteria | taxonomy | 출력 |
|---|---|---|---|---|
| **A. 레거시 정리** | 어느 모듈을 제거할지 / 의심 모듈 1개씩 (예: 20개) | 사용 빈도, 의존 반경, 제거 난이도, 신뢰도 | LIVE/SUPPORT/DORMANT/SCAFFOLD/DEAD (§2.4) | 제거 후보 ranked list + PR 분리 권고 |
| **B. 신기술 도입** | 도입할 기술 1~2개 선정 / 각 후보 기술 | stack 호환성, 성숙도, 도입 비용, 효과, lock-in | ADOPT/TRIAL/ASSESS/HOLD | 채택 + 시점 권고 + PoC 범위 |
| **C. 모델 후보** | PRIMARY 1 + FALLBACK 1 선정 / Qwen·GLM·Gemma 등 | VRAM fit, 한국어 품질, throughput, 라이선스, 커뮤니티 | PRIMARY/FALLBACK/EXPERIMENTAL/REJECTED | 모델 선택 + 배포 config + 벤치마크 |
| **D. 이슈 트리아지** | 오픈 이슈 N개 분류 / 각 이슈 | 사용자 영향, 수정 비용, 회귀 위험, 의존성 | CRITICAL/HIGH/MEDIUM/LOW/WONTFIX | 분류 + sprint 배정 + 차단/병합 그룹 |

### 시나리오 E: Mini Worked Example (E2E)
...
- [ ] `.analysis/<run-id>/00_plan.md` 작성 — 목적, criteria, taxonomy(정의 포함), scoring convention, topic list, guardrail check 확정
...
- [ ] `.analysis/<run-id>/` 디렉토리 생성, commit 정책 = default commit
- [ ] **공식 docs 대조** — 사용 중인 frontmatter 필드(`memory`, `isolation`, `model`)와 env var(`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS`)가 현재 Claude Code 버전에서 지원되는지 검증
- [ ] worktree 사용 시 동작 검증 (분기 후 worktree 목록 확인)
...
- [ ] dispatch ledger 확인 — 누락 topic 0건
- [ ] **분석 진행 중 메인 브랜치 변경 금지** — 동시 변경 시 baseline drift 발생

작업 후:

- [ ] `.analysis/<run-id>/` 산출물 git status clean (== 모두 commit)
- [ ] `00_manifest.json` 생성 — 사용된 agent 버전, 최종 topic 목록, baseline commit hash, 각 단계 timestamp, 모든 산출물 경로 기록
- [ ] **민감정보 grep 통과** — `.analysis/<run-id>/`에 secret/credential 패턴(AWS key regex, JWT, `password=` 등) 0건 확인
- [ ] Subagent memory에 도메인 패턴 누적 (durable pattern만):
      `@"topic-analyst (agent)" 이번 "<도메인>" 분석에서 발견한 <패턴 이름>을 장기 기억에 durable pattern으로 저장해줘.`
- [ ] 권고 채택 시 reversibility 태그를 실행 계획에 반영 (rollback 절차)
- [ ] PR description에 권고 trace 포함 — §10.1 PR template

### 10.1 PR description 템플릿 (권고 → 실행 trace)
...
- **Q1. 병렬 dispatch 중 일부 subagent가 조용히 실패하면?**
  → dispatch ledger의 status 컬럼이 빈 행 또는 `[SKIPPED]`로 남음. Step 4 진입 전 재-dispatch 또는 명시적 SKIPPED 마킹.
- **Q2. 점수 차이가 0.1로 미미한 경우?**
  → plan에서 선언한 |Δ| 임계(예: 0.5) 이내이므로 G3에서 정성 분석 단락 의무. 단순히 큰 쪽 채택하지 말 것.
- **Q3. memory에 뭐 저장?**
  → durable pattern만 (예: "Bash entry point 없는 Python module은 SCAFFOLD 후보일 수 있다"). run-specific 결론("M1은 DEAD")은 금지 (§6.5).
- **Q4. baseline이 너무 큰 경우?**
  → scope를 `full`에서 `medium`/`light`로 낮추거나, 시나리오에 맞게 항목 trim (§3.2 scope 표).
- **Q5. synthesis-reviewer를 안 쓰면?**
  → topic < 5이고 irreversible 권고 없고 외부 감사 대상 아니면 skip 가능. 단 G4는 자동으로 skip 처리, G3에서 reviewer 없이도 lint 룰(§7) 통과시켜야 함.
...
| **Evidence tag** | 모든 사실 주장에 부착하는 근거 태그. 형식 정본은 §2.1. |
...
| **Dispatch ledger** | 병렬 dispatch한 topic과 산출 파일을 대조하는 표 (§4 Step 3). |
...
- **다단계(hierarchical) 합성 패턴 (N>20)**: 그룹 합성기 → 최종 합성의 map-reduce 구조.
- **Wave dispatch + dry-run/예산 선언 (large N)**: 50개 단위 wave + 중간 commit, dispatch 전 토큰 예산 추정.
- **정형화된 오류 처리 계약**: 일시적 실패 vs 영구 실패 구분, 재시도 정책.
- **Re-analysis prior 입력 + diff (구조화)**: prior decay 정책, G3.5 게이트, 시간축 diff 분석.
...
tools: Read, Grep, Glob, WebFetch  # WebFetch는 re-fetch sampling용
model: sonnet
---

당신은 synthesis 결과의 비판적 리뷰어다. parent가 작성한 합성 문서를 받아 다음을 점검한다:

0. **Purpose alignment**: 권고가 plan의 목적/criteria를 모두 다루는가? plan이 stale이면 escalate.
1. **태그 일관성 — 무태그 문장 카운트 (lint)**: 모든 사실 주장에 evidence tag(규칙 §2.1), 모든 권고에 reversibility/confidence가 있는가? **무태그 사실 문장 1건이라도 있으면 blocking**.
2. **Webfetch re-fetch sampling**: webfetch tag의 access date + URL을 무작위 N개(예: 5개) 재-fetch하여 동일 사실 확인. 실패 시 즉시 `[ASSUMPTION]`으로 강등.
3. **논리 일관성**: 각 topic의 결과와 최종 합성의 권고가 일치하는가? 모순되는 곳은?
4. **누락된 trade-off**: 권고가 무시한 비용/리스크가 있는가?
5. **숨은 의존성**: cross-topic dependency에서 빠진 연결이 있는가? reciprocity check 결과는?
6. **확정성 과잉**: confidence가 Medium-Low인데 High처럼 단정한 문장이 있는가?
7. **Community tier 가중치 escalation**: community tier webfetch가 결정에 결정적 가중치를 가지면 plan 단계로 escalate.

## 입력
- `synthesis_path`: 검토할 합성 문서
- `plan_path`: 원본 plan (purpose alignment용)
